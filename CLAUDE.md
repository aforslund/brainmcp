...
- **`brain.ts`** — `Brain` class with all graph operations: `remember`, `recall` (with BFS 2-hop traversal), `associate`, `strengthen`, `weaken`, `reflect`, `forget`, `search`. All methods are synchronous (better-sqlite3 is sync).
...
- The database path defaults to `../brain.db` relative to `dist/`, which places it at the project root.
